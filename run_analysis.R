...
run_analysis <- function() {
  ...
  library(car)
  library(reshape2)
  X_train <- read.delim(paste(getwd(), "/UCI_HAR_Dataset/train/X_train.txt", header=F, sep="")
  Y_train <- read.delim(paste(getwd(), "/UCI_HAR_Dataset/train/y_train.txt", header=F, sep="")
  X_test <- read.delim(paste(getwd(), "/UCI_HAR_Dataset/test/X_test.txt", header=F, sep="")
  Y_test <- read.delim(paste(getwd(), "/UCI_HAR_Dataset/test/y_test.txt", header=F, sep="")
  varNames <- read.table(paste(getwd(), "/UCI_HAR_Dataset/features.txt", header=F, stringsAsFactors=F)
  subject_train <- read.table(paste(getwd(), "/UCI_HAR_Dataset/train/subject_train.txt", sep=""), header=F, stringsAsFactors=F)
  subject_test <- read.table(paste(getwd(), "/UCI_HAR_Dataset/test/subject_test.txt", header=F, stringsAsFactors=F)
